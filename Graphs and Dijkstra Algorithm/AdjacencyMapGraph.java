...
//  ----------------------------------------------------
  ...
  @Override
  public Vertex<V> first1() {
    return (Vertex<V>) vertices.first();
  }

  @Override
  public Vertex<V> last1() {
  ...
  public Vertex<V> opposite(String v, Edge<Integer> e) throws IllegalArgumentException {
    return null;
  }
...
//  start of the ds
  public void DFS( Graph<V,E> g, Vertex<V> start, Set<Vertex<V>> known, Map<Vertex<V>, Edge<E>> forest) {
    known.add(start);
    for (Edge<E> e: g.outgoingEdges(start)) {
      Vertex<V> v = g.opposite(start, e);
      if (!known.contains(v)) {
        forest.put(v, e);
